...
const GlobalStyle = () => (
  <style>{`
    ...
    @media (max-width: 768px) {
      ...
      .mp-main { padding: 1rem 1rem 5.5rem; }
...
const initialRecipes = [
  {
    id: "1", name: "Pâtes carbonara", category: "main",
    description: "Pâtes avec sauce carbonara maison",
    ingredients: [
      ...
      { ingredientId: "42", ingredientName: "Poivre", quantity: "au goût" },
    ],
    tags: ["Italien", "Rapide", "Famille"],
  },
  {
    id: "2", name: "Salade César", category: "starter",
    ...
    ingredients: [
      ...
      { ingredientId: "39", ingredientName: "Mayonnaise", quantity: "4 c. à soupe" },
      { ingredientId: "35", ingredientName: "Huile d'olive", quantity: "3 c. à soupe" },
    ],
    tags: ["Salade", "Léger", "Déjeuner"],
  },
  {
    id: "3", name: "Riz cantonais", category: "main",
    description: "Riz sauté avec légumes et œuf",
    ingredients: [
...
const RECIPE_CATEGORIES = [
  { id: "breakfast",  label: "Petit-déjeuner", icon: "☀️",  color: "amber",  hex: colors.amber },
  { id: "starter",    label: "Entrée",          icon: "🥗",  color: "sage",   hex: colors.sage },
  { id: "main",       label: "Plat principal",  icon: "🍽️",  color: "clay",   hex: colors.clay },
  { id: "soup",       label: "Soupe",           icon: "🥣",  color: "berry",  hex: colors.berry },
  { id: "snack",      label: "En-cas",          icon: "🥐",  color: "amber",  hex: "#B07A1A" },
  { id: "dessert",    label: "Dessert",         icon: "🍰",  color: "berry",  hex: "#9C3B4F" },
  { id: "sauce",      label: "Sauce / base",    icon: "🫙",  color: "sage",   hex: "#4A6340" },
  { id: "other",      label: "Autre",           icon: "📋",  color: "neutral", hex: colors.inkSoft },
];
  ...
  return (
    <>
      ...
      <nav className="mp-hide-mobile" style={{
        ...
        <div style={{ padding: "0 0.4rem", marginBottom: "2rem" }}>
          <h1 className="mp-h2" style={{ color: "var(--clay)" }}>Carnet</h1>
          <p className="mp-micro mp-text-faint" style={{ textTransform: "uppercase" }}>Planning familial</p>
        </div>
        ...
        {/* Séparateur + items secondaires */}
        <div style={{ marginTop: "1rem" }}>
          <hr className="mp-divider" />
          <div className="mp-nav-section-label">Paramètres</div>
          ...
          <div style={{ marginTop: "0.75rem", paddingTop: "0.75rem", borderTop: "1px solid var(--line)" }}>
            <button
              ...
              style={{
                ...
                border: "none", borderRadius: radius.sm, padding: "0.4rem 0.5rem",
            ...
            >
              <div style={{
                ...
                <span style={{ fontSize: "0.7rem", fontWeight: 700, color: "var(--clay)" }}>
                  {currentUser.name?.charAt(0).toUpperCase()}
                </span>
              </div>
              <div style={{ minWidth: 0 }}>
                <p className="mp-small" style={{ fontWeight: 600, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", color: "var(--ink)" }}>
                  {currentUser.name}
                </p>
            ...
            <button
              ...
              style={{ justifyContent: "flex-start", width: "100%", color: "var(--berry)", marginTop: "0.2rem" }}
              onClick={onLogout}
            >
              <Icon name="x" size={14} /> Se déconnecter
            </button>
      ...
      <nav className="mp-hide-desktop" style={{
          ...
          return (
            <button
              ...
              style={{
                ...
                background: "transparent",
                border: "none",
                color: active ? "var(--clay)" : "var(--ink-faint)",
                cursor: "pointer",
        ...
        <button
          ...
          style={{
            ...
            background: "transparent",
            border: "none",
            color: secondaryActive ? "var(--clay)" : "var(--ink-faint)",
            cursor: "pointer",
            transition: "color 100ms",
          }}
          aria-label="Plus"
        >
          {/* Icône "…" — 3 points horizontaux */}
...
const DayPanel = ({
  ...
  onDuplicateWeek, onApplyTemplate,
}) => {
  ...
  const handleSaveSlot = (recipeIds) => {
    ...
    if (meal) onUpdateMeal(meal.id, recipeIds);
    else onAddMeal({ date: dateStr, type, recipeIds });
    setEditingSlot(null);
    setFlashedSlot(type);
    setTimeout(() => setFlashedSlot(null), 650);
  };
    ...
    <div style={{
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "0.75rem", flexWrap: "wrap", gap: "0.5rem" }}>
        ...
        <div style={{ display: "flex", gap: "0.4rem", flexWrap: "wrap" }}>
          ...
          >
            <Icon name="copy" size={13} /> Dupliquer la semaine
          </button>
          ...
          {weekTemplates.length > 0 && (
            <div style={{ position: "relative" }}>
              ...
              >
                <Icon name="calendar" size={13} /> Appliquer un modèle
              </button>
              {showTemplates && (
                <div style={{
                  position: "absolute",
                  ...
                  {weekTemplates.map((tpl) => (
                    <button
                      ...
                      onClick={() => {
                        const monday = getMondayOf(new Date(dateStr + "T12:00:00"));
                        onApplyTemplate(tpl, monday.toISOString().split("T")[0], "merge");
        ...
        {MEAL_TYPES.map((type) => {
          ...
          return (
            <button
              ...
              style={{
                background: names.length > 0 ? washColor : "var(--paper-sunken)",
                border: `1px solid ${names.length > 0 ? dotColor : "var(--line)"}`,
      ...
      {showDuplicate && (
        <DuplicateWeekModal
          ...
          onDuplicate={(srcDateStr, targetMondayStr) => {
            onDuplicateWeek(srcDateStr, targetMondayStr);
          }}
        />
      )}
    </div>
...
const CalendarView = ({ mealPlans, recipes, onAddMeal, onUpdateMeal, recentRecipeIds = [], weekTemplates = [], onApplyTemplate, onDuplicateWeek, onClearWeek, onNavigate }) => {
  const [viewMode, setViewMode] = useState("month");
  const [currentDate, setCurrentDate] = useState(new Date());
  const [selectedDate, setSelectedDate] = useState(null);     // vue mois — jour sélectionné
  const [weekEditingSlot, setWeekEditingSlot] = useState(null); // vue semaine — créneau en édition
  const [weekActionType, setWeekActionType] = useState(null);   // "duplicate" | "clear" | "apply-template" | null
  const [weekActionDate, setWeekActionDate] = useState(null);   // dateStr source
  const [weekActionTemplate, setWeekActionTemplate] = useState(null); // template sélectionné
    ...
    <div>
      ...
      <div className="mp-view-header">
        <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", flexWrap: "wrap" }}>
          ...
          <h2 className="mp-h2" style={{ minWidth: "9rem" }}>
            {viewMode === "month"
        ...
        <div style={{ display: "flex", gap: "0.4rem" }}>
          <button type="button" className={`mp-btn mp-btn-sm ${viewMode === "week" ? "mp-btn-primary" : "mp-btn-secondary"}`} onClick={() => setViewMode("week")}>
            Semaine
          </button>
          <button type="button" className={`mp-btn mp-btn-sm ${viewMode === "month" ? "mp-btn-primary" : "mp-btn-secondary"}`} onClick={() => setViewMode("month")}>
            Mois
          </button>
        </div>
      </div>
      ...
      {viewMode === "week" && calendarWeeks[0] && (() => {
        ...
        return (
          <div>
            {/* Barre d'actions semaine — hors des cartes */}
            <div style={{ display: "flex", gap: "0.5rem", justifyContent: "flex-end", marginBottom: "0.75rem", flexWrap: "wrap" }}>
              {/* Vider la semaine */}
              <button
                type="button"
                className="mp-btn mp-btn-secondary mp-btn-sm"
                style={{ color: "var(--berry)", borderColor: "var(--berry-wash)" }}
                onClick={() => { setWeekActionDate(weekDateStr); setWeekActionType("clear"); }}
              >
                <Icon name="trash" size={13} /> Vider la semaine
              </button>
              ...
              {/* Appliquer un modèle — toujours visible même si aucun modèle */}
              <div style={{ position: "relative" }}>
                <button
                  type="button"
                  className="mp-btn mp-btn-secondary mp-btn-sm"
                ...
                {showWeekTemplates && (
                  <div style={{
                    ...
                    {weekTemplates.length === 0 && (
                      <p className="mp-small mp-text-faint" style={{ padding: "0.6rem 0.85rem" }}>
                    ...
                    {weekTemplates.map((tpl) => (
                      <button
                        ...
                        type="button"
                        onClick={() => {
                          setWeekActionDate(weekDateStr);
                          setWeekActionType("apply-template");
                          setWeekActionTemplate(tpl);
                    ...
                    <button
                      ...
                      style={{
                        ...
                        background: "var(--paper-sunken)", border: "none",
                        borderTop: weekTemplates.length > 0 ? "1px solid var(--line)" : "none",
                        cursor: "pointer", textAlign: "left", fontFamily: "inherit",
                        fontSize: "0.82rem", color: "var(--clay)", fontWeight: 600,
                      }}
                      onMouseEnter={(e) => e.currentTarget.style.background = "var(--clay-wash)"}
                ...
                return (
                  <React.Fragment key={dateStr}>
                    ...
                    <div style={{
                      ...
                      <span className="mp-week-day-label" style={{
                        color: isToday ? "rgba(255,255,255,0.75)" : "var(--ink-faint)",
                      }}>
                        {dayLabel}
                      </span>
                      <span className="mp-week-day-num" style={{
                    ...
                    {MEAL_TYPES.map((type) => {
                      ...
                      const names = (meal?.recipeIds || [])
                        .map((id) => recipes.find((r) => r.id === id)?.name)
                        .filter(Boolean);
                      const dotColor = type.color === "amber" ? "var(--amber)"
                        : type.color === "clay" ? "var(--clay)" : "var(--sage)";
                      const washColor = type.color === "amber" ? "var(--amber-wash)"
                        : type.color === "clay" ? "var(--clay-wash)" : "var(--sage-wash)";
                      ...
                      return (
                        <button
                          ...
                          style={{
                            ...
                            background: names.length > 0 ? washColor : "var(--paper-raised)",
                            border: `1px solid ${names.length > 0 ? dotColor : "var(--line)"}`,
                            borderRadius: radius.sm,
            ...
            {weekEditingSlot && (
              <RecipeSelectionModal
                ...
                onSave={(recipeIds) => {
                  ...
                  if (meal) onUpdateMeal(meal.id, recipeIds);
                  else onAddMeal({ date: weekEditingSlot.dateStr, type: weekEditingSlot.type, recipeIds });
...
const RecipeSelectionModal = ({ recipes, meal, mealType, date, onClose, onSave, recentRecipeIds = [] }) => {
  ...
  // Catégories présentes dans les recettes
  ...
  const RecipeRow = ({ recipe }) => {
    ...
    return (
      <label style={{
        ...
        {cat && <span style={{ fontSize: "1rem", lineHeight: 1, flexShrink: 0 }}>{cat.icon}</span>}
  ...
  return (
    <Modal onClose={onClose} width="420px">
      <ModalHeader title="Choisir des recettes" onClose={onClose} />
      ...
      {/* Liste */}
      <div style={{ maxHeight: "300px", overflowY: "auto", marginBottom: space.md }}>
      ...
      <div style={{ display: "flex", gap: "0.6rem", justifyContent: "flex-end" }}>
        <button type="button" className="mp-btn mp-btn-secondary" onClick={onClose}>Annuler</button>
        <button type="button" className="mp-btn mp-btn-primary" onClick={() => onSave(selected)}>
...
const RecipeModal = ({ recipe, ingredients, onClose, onSave }) => {
  ...
  const handleSave = () => {
    if (!name.trim()) return;
    onSave({ id: recipe?.id || Date.now().toString(), name: name.trim(), category, description: description.trim(), ingredients: recipeIngredients, tags });
  };
  ...
  return (
    <Modal onClose={onClose} width="500px">
      <ModalHeader title={recipe ? "Modifier la recette" : "Nouvelle recette"} onClose={onClose} />
...
const RecipeDetailModal = ({ recipe, ingredients, onClose, onEdit }) => {
  const cat = RECIPE_CATEGORIES.find((c) => c.id === recipe.category);

  // Grouper les ingrédients par catégorie d'ingrédient
    ...
    <Modal onClose={onClose} width="480px">
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: space.lg }}>
        <div style={{ display: "flex", gap: "0.75rem", alignItems: "center" }}>
          {cat && <span style={{ fontSize: "2rem", lineHeight: 1 }}>{cat.icon}</span>}
          <div>
            <h2 className="mp-h2" style={{ marginBottom: "0.3rem" }}>{recipe.name}</h2>
            {cat && (
              <span className="mp-badge" style={{ background: `${cat.hex}18`, color: cat.hex, border: `1px solid ${cat.hex}30` }}>
                {cat.label}
              </span>
            )}
          </div>
      ...
      {recipe.tags.length > 0 && (
        <div style={{ display: "flex", flexWrap: "wrap", gap: "0.3rem", marginBottom: space.lg }}>
      ...
      <div style={{ display: "flex", gap: "0.6rem", justifyContent: "flex-end", marginTop: space.xl, paddingTop: space.lg, borderTop: "1px solid var(--line)" }}>
...
const RecipesView = ({ recipes, ingredients, onAddRecipe, onEditRecipe, onDeleteRecipe }) => {
  ...
  const handleSave = (recipeData) => {
    if (modalRecipe) onEditRecipe(recipeData);
  ...
  return (
    <div>
      <div className="mp-view-header">
        <h1 className="mp-h1">
          Recettes{" "}
          <span className="mp-small mp-text-soft" style={{ fontWeight: 400, fontFamily: "inherit" }}>({recipes.length})</span>
        </h1>
        <div style={{ display: "flex", gap: "0.4rem" }}>
        ...
        {RECIPE_CATEGORIES.map((cat) => {
          ...
          return (
            <button key={cat.id} type="button" onClick={() => setFilterCategory(filterCategory === cat.id ? null : cat.id)} className="mp-small"
              ...
              {cat.icon} {cat.label} <span style={{ opacity: 0.7 }}>·{count}</span>
            </button>
          );
        })}
      </div>
            ...
            return (
              <div key={recipe.id} className="mp-card mp-recipe-card"
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "0.5rem" }}>
                  {cat && <span style={{ fontSize: "1.3rem", lineHeight: 1, flexShrink: 0 }}>{cat.icon}</span>}
                  <h3 className="mp-h3" style={{ margin: 0, paddingRight: "3.5rem" }}>{recipe.name}</h3>
                </div>
          ...
          {filtered.map((recipe) => {
            ...
            return (
              <div key={recipe.id} onClick={() => setDetailRecipe(recipe)}
                ...
                onMouseLeave={(e) => e.currentTarget.style.background = "transparent"}>
                <span style={{ fontSize: "1.1rem", lineHeight: 1, flexShrink: 0, width: "1.4rem", textAlign: "center" }}>{cat?.icon || "📋"}</span>
                <span className="mp-small" style={{ fontWeight: 600, flex: 1 }}>{recipe.name}</span>
                {cat && (
                  <span className="mp-badge" style={{ background: `${cat.hex}18`, color: cat.hex, border: `1px solid ${cat.hex}30`, fontSize: "0.6rem", fontWeight: 600, flexShrink: 0 }}>
                    {cat.label}
                  </span>
                )}
...
const WeekTemplateEditor = ({ template, recipes, recentRecipeIds, familyMembers, onSave, onCancel }) => {
  const [name, setName] = useState(template?.name || "Ma semaine type");
  const [memberId, setMemberId] = useState(template?.memberId ?? null);
    ...
    <div className="mp-card" style={{ background: "var(--paper-sunken)" }}>
      <div style={{ display: "flex", gap: "0.7rem", flexWrap: "wrap", marginBottom: space.md, alignItems: "flex-end" }}>
        <div style={{ flex: "1 1 200px" }}>
          <span className="mp-label">Nom du template</span>
          <input
            className="mp-input"
            value={name}
            onChange={(e) => setName(e.target.value)}
            placeholder="Ex : Semaine classique"
          />
        </div>
        <div style={{ flex: "1 1 160px" }}>
          <span className="mp-label">Membre associé</span>
          <select className="mp-select" value={memberId ?? ""} onChange={(e) => setMemberId(e.target.value || null)}>
...
const TagList = ({ items, variant, emptyLabel }) => (
  <div style={{ display: "flex", flexWrap: "wrap", gap: "0.3rem" }}>
    {items.length > 0
      ? items.map((item) => <span key={item} className={`mp-badge mp-badge-${variant}`}>{item}</span>)
      : <span className="mp-small mp-text-faint">{emptyLabel}</span>}
  </div>
);

const MemberCard = ({ member, onEdit, onDelete }) => (
  <div className="mp-card" style={{ display: "flex", flexDirection: "column", gap: "0.6rem" }}>
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
      <div>
        <h3 className="mp-h3">{member.name}</h3>
        <p className="mp-small mp-text-soft">{member.email}</p>
      </div>
      <div style={{ display: "flex", gap: "0.2rem" }}>
        <button type="button" className="mp-btn mp-btn-ghost mp-btn-icon" onClick={() => onEdit(member.id)} aria-label="Modifier">
          <Icon name="edit" size={14} />
        </button>
        <button type="button" className="mp-btn mp-btn-danger mp-btn-icon" onClick={() => onDelete(member.id)} aria-label="Supprimer">
          <Icon name="trash" size={14} />
        </button>
      </div>
    </div>
    <div>
      <p className="mp-micro mp-text-soft" style={{ textTransform: "uppercase", fontWeight: 600, marginBottom: "0.3rem" }}>Préférences</p>
      <TagList items={member.preferences} variant="sage" emptyLabel="Aucune" />
    </div>
    <div>
      <p className="mp-micro mp-text-soft" style={{ textTransform: "uppercase", fontWeight: 600, marginBottom: "0.3rem" }}>Allergies</p>
      <TagList items={member.allergies} variant="berry" emptyLabel="Aucune" />
    </div>
  </div>
);

const PreferencesView = ({ familyMembers, ingredients, weekTemplates, recipes, recentRecipeIds,
  ...
  // Semaine type
  const [editingTemplate, setEditingTemplate] = useState(undefined); // undefined=fermé, null=nouveau, obj=édition
  ...
  return (
    <div>
      <div className="mp-view-header">
        ...
        <button type="button" className="mp-btn mp-btn-primary" onClick={onAddMember}>
          <Icon name="plus" size={15} /> Membre
        </button>
      </div>
      ...
      <div className="mp-grid-cards">
        {familyMembers.map((member) => (
          <MemberCard key={member.id} member={member} onEdit={onEditMember} onDelete={onDeleteMember} />
        ))}
        {familyMembers.length === 0 && (
          <div style={{ gridColumn: "1 / -1" }}>
            <EmptyState title="Aucun membre ajouté" hint="Ajoutez des membres de la famille" />
          </div>
        )}
      </div>

      {/* ---- Semaines types ---- */}
      <div style={{ marginTop: space["2xl"], paddingTop: space.xl, borderTop: "1px solid var(--line)" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: space.lg, flexWrap: "wrap", gap: "0.6rem" }}>
          <div>
            <h2 className="mp-h2">Semaines types</h2>
            <p className="mp-small mp-text-soft" style={{ marginTop: "0.2rem" }}>
              Définissez des modèles de repas à appliquer en un clic à n'importe quelle semaine.
            </p>
          </div>
          <button
            type="button"
        ...
        {editingTemplate !== undefined && (
          <div style={{ marginBottom: space.lg }}>
            <WeekTemplateEditor
              ...
              onSave={(tpl) => { onSaveTemplate(tpl); setEditingTemplate(undefined); }}
        ...
        {weekTemplates.length === 0 && editingTemplate === undefined && (
          <EmptyState
            title="Aucun modèle de semaine"
            hint="Créez un modèle pour planifier rapidement vos repas récurrents."
          />
        )}

        <div style={{ display: "flex", flexDirection: "column", gap: space.lg }}>
          {weekTemplates.map((tpl) => {
            const member = familyMembers.find((m) => m.id === tpl.memberId);
                  ...
                  <div>
                    ...
                    <div style={{ display: "flex", gap: "0.4rem", marginTop: "0.3rem", flexWrap: "wrap" }}>
                      <span className="mp-badge mp-badge-neutral">
                        {filledCount} créneau{filledCount > 1 ? "x" : ""}
                      </span>
                      {member
                        ? <span className="mp-badge mp-badge-sage">{member.name}</span>
                        : <span className="mp-badge mp-badge-neutral">Commun</span>
                      }
                    </div>
                  </div>
                  <div style={{ display: "flex", gap: "0.4rem" }}>
                    ...
                    <button
                      type="button"
                      className="mp-btn mp-btn-danger mp-btn-icon"
      ...
      <div style={{ marginTop: space["2xl"], paddingTop: space.xl, borderTop: "1px solid var(--line)" }}>
          ...
          {ingredientCategories.map((cat) => (
            <button
              ...
              style={{
                ...
                background: selectedCategory === cat.id ? cat.hex : "transparent",
                color: selectedCategory === cat.id ? "#fff" : cat.hex,
                cursor: "pointer",
                fontWeight: 500,
              }}
            >
        ...
        <input
          className="mp-input"
          style={{ maxWidth: "320px", marginBottom: space.md }}
          value={search}
      ...
      {applyingTemplate && (
        <ApplyTemplateModal
          ...
          onApply={(weekStart, mode) => {
            onApplyTemplate(applyingTemplate, weekStart, mode);
...
const AccountView = ({ currentUser, onLogout }) => {
    ...
    <div>
      ...
      <div className="mp-card" style={{ display: "flex", alignItems: "center", gap: "1.25rem", marginBottom: space.xl, flexWrap: "wrap" }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          {isDemo && (
            <span className="mp-badge mp-badge-amber" style={{ marginTop: "0.4rem" }}>Compte de démonstration</span>
          )}
        </div>
      </div>
      ...
      <div className="mp-card" style={{ border: "1px solid var(--berry-wash)" }}>
        ...
        {!showConfirm ? (
          ...
          <div style={{ display: "flex", gap: "0.6rem", alignItems: "center", flexWrap: "wrap" }}>
            <span className="mp-small mp-text-soft">Confirmer la déconnexion ?</span>
            <button type="button" className="mp-btn mp-btn-secondary mp-btn-sm" onClick={() => setShowConfirm(false)}>
              Annuler
            </button>
            <button
              type="button"
...
const LinkMemberModal = ({ registeredUsers, familyMembers, onClose, onLink }) => {
  const [selected, setSelected] = useState(null);

  // Comptes disponibles = tous les comptes (démo inclus) pas encore dans la famille
  const allAccounts = [
    { id: DEMO_USER.id, name: DEMO_USER.name, email: DEMO_USER.email },
    ...registeredUsers.map(({ password: _, ...u }) => u),
  ];
  const linkedEmails = new Set(familyMembers.map((m) => m.email?.toLowerCase()).filter(Boolean));
  const available = allAccounts.filter((u) => !linkedEmails.has(u.email.toLowerCase()));

  return (
    <Modal onClose={onClose} width="400px">
      <ModalHeader title="Associer un compte" onClose={onClose} />
      <p className="mp-small mp-text-soft" style={{ marginBottom: space.md }}>
        Choisissez un compte existant à associer à votre famille.
      </p>

      {available.length === 0 ? (
        <EmptyState title="Aucun compte disponible" hint="Tous les comptes enregistrés sont déjà membres." />
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: "0.3rem", marginBottom: space.lg }}>
          {available.map((user) => (
            <label
              key={user.id}
              style={{
                ...
                borderRadius: radius.sm,
                cursor: "pointer",
                background: selected?.id === user.id ? "var(--clay-wash)" : "transparent",
                border: `1px solid ${selected?.id === user.id ? "var(--clay-soft)" : "transparent"}`,
                transition: "background 100ms",
              }}
            >
              <input
              ...
              <div style={{
                ...
                background: "var(--clay-wash)", display: "flex", alignItems: "center",
                justifyContent: "center", flexShrink: 0,
              }}>
                <span style={{ fontSize: "0.75rem", fontWeight: 700, color: "var(--clay)" }}>
                  {user.name.charAt(0).toUpperCase()}
                </span>
              </div>
              <div>
                <p className="mp-small" style={{ fontWeight: 500 }}>{user.name}</p>
                <p className="mp-micro mp-text-faint">{user.email}</p>
              </div>
            </label>
          ))}
        </div>
...
const FamilyView = ({ familyMembers, currentUser, registeredUsers, onLinkMember, onEditMember, onDeleteMember }) => {
  ...
  return (
    <div>
      <div className="mp-view-header">
        ...
        <button type="button" className="mp-btn mp-btn-primary" onClick={() => setShowLinkModal(true)}>
          <Icon name="plus" size={15} /> Associer un compte
        </button>
      </div>

      <h3 className="mp-h3" style={{ marginBottom: space.md }}>Membres ({familyMembers.length})</h3>
      <div className="mp-grid-cards">
        {familyMembers.map((member) => {
          const isCurrentUser = currentUser?.email?.toLowerCase() === member.email?.toLowerCase();
          return (
            <div key={member.id} className="mp-card" style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: "0.6rem", flexWrap: "wrap" }}>
              <div style={{ display: "flex", alignItems: "center", gap: "0.6rem" }}>
                <div style={{
                  ...
                  <span style={{ fontSize: "0.8rem", fontWeight: 700, color: "var(--clay)" }}>
                    {member.name.charAt(0).toUpperCase()}
                  </span>
                </div>
                <div>
                  <div style={{ display: "flex", alignItems: "center", gap: "0.4rem" }}>
                    <p className="mp-small" style={{ fontWeight: 600 }}>{member.name}</p>
                    {isCurrentUser && <span className="mp-badge mp-badge-clay">Vous</span>}
                  </div>
                  <p className="mp-micro mp-text-faint">{member.email}</p>
                </div>
              </div>
              <div style={{ display: "flex", gap: "0.4rem", alignItems: "center" }}>
                <span className={`mp-badge ${member.preferences?.length > 0 ? "mp-badge-sage" : "mp-badge-neutral"}`}>
                  {member.preferences?.length || 0} préf.
                </span>
                <span className={`mp-badge ${member.allergies?.length > 0 ? "mp-badge-berry" : "mp-badge-neutral"}`}>
                  {member.allergies?.length || 0} allerg.
                </span>
                {!isCurrentUser && (
                  <>
                    <button type="button" className="mp-btn mp-btn-ghost mp-btn-icon" onClick={() => onEditMember(member.id)} aria-label="Modifier">
                      <Icon name="edit" size={14} />
                    </button>
                    <button type="button" className="mp-btn mp-btn-danger mp-btn-icon" onClick={() => onDeleteMember(member.id)} aria-label="Retirer">
                      <Icon name="trash" size={14} />
                    </button>
                  </>
                )}
        ...
        {familyMembers.length === 0 && (
          <div style={{ gridColumn: "1 / -1" }}>
            <EmptyState title="Aucun membre associé" hint="Associez des comptes existants pour partager le planning." />
          </div>
        )}
      </div>
...
const AuthLogo = () => (
  <div className="mp-auth-logo">
    <div className="mp-h1 mp-serif" style={{ color: "var(--clay)", marginBottom: "0.2rem" }}>Carnet</div>
    <p className="mp-micro mp-text-faint" style={{ textTransform: "uppercase", letterSpacing: "0.08em" }}>Planning familial</p>
  </div>
);
...
const STORAGE_KEYS = {
  ...
  familyMembers: "mealPlanner_familyMembers",
...
const App = () => {
  ...
  const [familyMembers, setFamilyMembers] = useState(() => loadFromStorage(STORAGE_KEYS.familyMembers, initialFamilyMembers));
  ...
  // Liste des comptes enregistrés (sans mots de passe) pour FamilyView
  const registeredUsers = useMemo(() =>
    loadFromStorage(STORAGE_KEYS.registeredUsers, []).map(({ password: _, ...u }) => u),
  [familyMembers]); // recalculé quand la famille change

  // 5 recettes les plus récemment planifiées (IDs uniques, ordre chronologique inverse)
  const recentRecipeIds = useMemo(() => {
    const sorted = [...mealPlans].sort((a, b) => b.date.localeCompare(a.date));
    const seen = new Set();
  ...
  useEffect(() => saveToStorage(STORAGE_KEYS.familyMembers, familyMembers), [familyMembers]);
  ...
  // ---- Auth handlers ----
  // TODO Phase 3 — remplacer ces handlers par des appels fetch vers /api/auth/*
  // Exemple : const res = await fetch("/api/auth/login", { method:"POST", body: JSON.stringify({email,password}) })

  const handleLogin = async (email, password) => {
    // Compte de démonstration — toujours valide
  ...
  const handleAddRecipe = (recipe) => setRecipes((prev) => [...prev, recipe]);
  const handleEditRecipe = (updated) => setRecipes((prev) => prev.map((r) => (r.id === updated.id ? updated : r)));
  ...
  const handleAddMeal = (mealData) => {
    setMealPlans((prev) => [
      ...prev,
      {
        ...
        date: mealData?.date || todayStr(),
        recipeIds: mealData?.recipeIds || [],
        type: mealData?.type || "lunch",
      },
    ]);
  };
  const handleUpdateMeal = (mealId, recipeIds) =>
    setMealPlans((prev) => prev.map((mp) => (mp.id === mealId ? { ...mp, recipeIds } : mp)));

  const handleAddShoppingItem = (item) =>
    setShoppingList((prev) => [...prev, { id: Date.now().toString(), name: item.name, quantity: item.quantity, completed: false }]);
  const handleToggleShoppingItem = (id) =>
    setShoppingList((prev) => prev.map((item) => (item.id === id ? { ...item, completed: !item.completed } : item)));
  const handleDeleteShoppingItem = (id) => setShoppingList((prev) => prev.filter((item) => item.id !== id));

  // Agrège les ingrédients des recettes planifiées à partir d'aujourd'hui (7 jours)
  // et ajoute ceux qui ne sont pas déjà présents dans la liste (par nom).
  const handleGenerateShoppingList = (from, to) => {
    const startStr = from || todayStr();
    const endDate = new Date((to || startStr) + "T12:00:00");
    endDate.setDate(endDate.getDate() + 1); // inclusif
    const endStr = endDate.toISOString().split("T")[0];

    const upcomingMeals = mealPlans.filter((mp) => mp.date >= startStr && mp.date < endStr);

    if (upcomingMeals.length === 0) {
      showToast(`Aucune recette planifiée entre le ${startStr} et le ${to || startStr}`, "berry");
      return;
    }
    ...
    upcomingMeals.forEach((meal) => {
      const ids = meal.recipeIds || (meal.recipeId ? [meal.recipeId] : []);
      ids.forEach((recipeId) => {
        ...
        recipe.ingredients.forEach((ing) => {
          ...
          if (!aggregated.has(key)) aggregated.set(key, new Set());
          aggregated.get(key).add(ing.quantity);
        });
      });
    });
    ...
    setShoppingList((prev) => {
      ...
      const additions = [];
      aggregated.forEach((quantities, name) => {
        if (existingNames.has(name.toLowerCase())) return;
        additions.push({
          id: `${Date.now()}-${name}`,
          name,
          quantity: Array.from(quantities).join(" + "),
          completed: false,
        });
      });
    ...
    // Le setState est async, on lit addedCount depuis la closure
    setTimeout(() => {
      if (addedCount === 0) {
        showToast("Tous les ingrédients sont déjà dans la liste", "sage");
      } else {
        showToast(`${addedCount} article${addedCount > 1 ? "s" : ""} ajouté${addedCount > 1 ? "s" : ""} depuis ${recipeCount} recette${recipeCount > 1 ? "s" : ""}`, "sage");
      }
    }, 0);
  };

  const handleAddFamilyMember = () => setEditingMemberId(NEW_MEMBER_SENTINEL);
  const handleSaveFamilyMember = (updated) => {
    if (updated.id === NEW_MEMBER_SENTINEL) {
      setFamilyMembers((prev) => [...prev, { ...updated, id: Date.now().toString() }]);
    } else {
      setFamilyMembers((prev) => prev.map((m) => (m.id === updated.id ? updated : m)));
    }
  };

  // Associer un compte existant comme membre de la famille
  const handleLinkMember = (user) => {
    const alreadyLinked = familyMembers.some((m) => m.email?.toLowerCase() === user.email.toLowerCase());
    ...
    showToast(`${user.name} ajouté à la famille`, "sage");
  };
  const handleDeleteFamilyMember = (id) => setFamilyMembers((prev) => prev.filter((m) => m.id !== id));

  const handleAddIngredient = (newIngredient) => setIngredients((prev) => [...prev, newIngredient]);
  const handleDeleteIngredient = (id) => {
    const isUsed = recipes.some((r) => r.ingredients?.some((ing) => ing.ingredientId === id));
    if (isUsed) {
      alert("Cet ingrédient est utilisé dans une recette et ne peut pas être supprimé.");
      return;
    }
  ...
  const handleApplyTemplate = (template, weekStart, mode) => {
    ...
    setMealPlans((prev) => {
      // En mode "overwrite" : supprimer les plans existants sur les 7 jours couverts par le template
      let base = prev;
      if (mode === "overwrite") {
        const affectedDates = new Set(
          template.slots.map((s) => dateOfSlot(monday, s.day))
        );
        base = prev.filter(
          (mp) => !affectedDates.has(mp.date) ||
            !template.slots.some((s) => s.type === mp.type && dateOfSlot(monday, s.day) === mp.date)
        );
      }
      // Ajouter les créneaux du template
      const additions = template.slots
        .filter((slot) => {
          if (mode === "merge") {
            const date = dateOfSlot(monday, slot.day);
            return !base.some((mp) => mp.date === date && mp.type === slot.type && (mp.recipeIds || []).length > 0);
          }
          return true;
        })
        .map((slot) => ({
          id: `tpl-${Date.now()}-${slot.day}-${slot.type}`,
          date: dateOfSlot(monday, slot.day),
          type: slot.type,
    ...
    showToast(`${count} créneau${count > 1 ? "x" : ""} appliqué${count > 1 ? "s" : ""} — semaine du ${weekStart}`, "sage");
  };

  // Duplique tous les repas de la semaine contenant srcDateStr vers la semaine de targetMondayStr
  const handleDuplicateWeek = (srcDateStr, targetMondayStr) => {
    ...
    if (weekMeals.length === 0) {
      showToast("Aucun repas à dupliquer cette semaine", "berry");
      return;
    }
    ...
    setMealPlans((prev) => {
      const additions = weekMeals.map((mp) => {
        ...
        newDate.setDate(targetMonday.getDate() + offset);
        const newDateStr = newDate.toISOString().split("T")[0];
        const exists = prev.some((p) => p.date === newDateStr && p.type === mp.type && (p.recipeIds||[]).length > 0);
        if (exists) return null;
        return { id: `dup-${Date.now()}-${mp.id}`, date: newDateStr, type: mp.type, recipeIds: [...(mp.recipeIds || [])] };
      }).filter(Boolean);
      return [...prev, ...additions];
    });

    showToast(`${weekMeals.length} repas dupliqué${weekMeals.length > 1 ? "s" : ""} → semaine du ${targetMondayStr}`, "sage");
  };
  ...
  const handleClearWeek = (dateStr) => {
    ...
    const mondayStr = monday.toISOString().split("T")[0];
    setMealPlans((prev) => {
      const removed = prev.filter((mp) => {
        const m = getMondayOf(new Date(mp.date + "T12:00:00"));
        return m.toISOString().split("T")[0] === mondayStr;
      });
      showToast(`${removed.length} repas supprimé${removed.length > 1 ? "s" : ""}`, "berry");
      return prev.filter((mp) => {
        const m = getMondayOf(new Date(mp.date + "T12:00:00"));
        return m.toISOString().split("T")[0] !== mondayStr;
      });
    });
  ...
  const viewProps = {
    calendar: { mealPlans, recipes, onAddMeal: handleAddMeal, onUpdateMeal: handleUpdateMeal, recentRecipeIds, weekTemplates, onApplyTemplate: handleApplyTemplate, onDuplicateWeek: handleDuplicateWeek, onClearWeek: handleClearWeek, onNavigate: setCurrentView },
    recipes: { recipes, ingredients, onAddRecipe: handleAddRecipe, onEditRecipe: handleEditRecipe, onDeleteRecipe: handleDeleteRecipe },
    shopping: {
      shoppingList,
      ingredients,
    ...
    preferences: {
      familyMembers, ingredients, weekTemplates, recipes, recentRecipeIds,
      onAddMember: handleAddFamilyMember, onEditMember: setEditingMemberId, onDeleteMember: handleDeleteFamilyMember,
      onAddIngredient: handleAddIngredient, onDeleteIngredient: handleDeleteIngredient,
      onSaveTemplate: handleSaveTemplate, onDeleteTemplate: handleDeleteTemplate, onApplyTemplate: handleApplyTemplate,
    },
    family: { familyMembers, currentUser, registeredUsers, onLinkMember: handleLinkMember, onEditMember: setEditingMemberId, onDeleteMember: handleDeleteFamilyMember },
    account: { currentUser, onLogout: handleLogout },
  };
  ...
  return (
    <div className={`mp-root${darkMode ? " dark" : ""}`}>
      ...
      {!currentUser && authScreen === "forgot" && (
        <ForgotPasswordView onGoLogin={() => setAuthScreen("login")} />
      )}
      ...
      {currentUser && (
        <>
          ...
          <button
            type="button"
            className="mp-fab mp-hide-desktop"
          ...
          {showFab && (
            <QuickPlanModal
              ...
              onSave={(mealData) => {
                ...
                setShowFab(false);
                showToast(`Repas planifié le ${mealData.date}`);
              }}
            />
          )}
          ...
          {editingMemberId && (
            <MemberModal
              member={
                editingMemberId === NEW_MEMBER_SENTINEL
                  ? { id: NEW_MEMBER_SENTINEL, name: "", email: "", preferences: [], allergies: [] }
                  : familyMembers.find((m) => m.id === editingMemberId)
              }
              onClose={() => setEditingMemberId(null)}
              onSave={(updated) => {
                handleSaveFamilyMember(updated);
